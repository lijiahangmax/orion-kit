...
class BitsCronField extends CronField {
    ...
    /**
     * 我们最多存储 60 位 用于秒和分钟 所以 64 位长就足够了
     */
    private long bits;
    ...
    /*** 返回启用了 0 纳秒的 {@code BitsCronField}  */
    ...
    /**
     * 将给定值解析为秒 {@code BitsCronField} cron 表达式的第一个条目
     */
    ...
    /**
     * 将给定值解析为分钟 {@code BitsCronField} cron 表达式的第二个条目
     */
    ...
    /**
     * 将给定值解析为小时 {@code BitsCronField} cron 表达式的第三个条目
     */
    ...
    /**
     * 将给定的值解析为一个月的天数 {@code BitsCronField} cron 表达式的第四个条目
     */
    ...
    /**
     * 将给定的值解析为一个月 {@code BitsCronField} cron 表达式的第五个条目
     */
    ...
    /**
     * 将给定的值解析为星期几 {@code BitsCronField} cron 表达式的第六个条目
     */
    ...
    private static BitsCronField parseField(String value, Type type) {
        ...
        Valid.notNull(type, "type must not be null");
        try {
            ...
            for (String field : fields) {
                ...
                if (slashPos == -1) {
                    ...
                    if (delta <= 0) {
                        throw Exceptions.argument("incrementer delta must be 1 or higher");
                    }
    ...
    public <T extends Temporal & Comparable<? super T>> T nextOrSame(T temporal) {
        ...
        if (next == current) {
            ...
            while (current != next && count++ < Cron.MAX_ATTEMPTS) {
